...
# --change_qk --relative_position --mode super --dist-eval --cfg ./experiments/supernet/supernet-T.yaml --epochs 501 --warmup-epochs 20 \
# --resume '/OUTPUT_PATH/checkpoint-tiny-only-supernet-minimum-25.pth' --output /OUTPUT_PATH --batch-size 128 \
# --save_checkpoint_path 'checkpoint-tiny-only-supernet-minimum-' --save_log_path './log/feature_extracting.log'

python -m torch.distributed.launch --nproc_per_node=8 --use_env supernet_train_gradient.py --data-path '/data' --gp \
--change_qk --relative_position --mode super --dist-eval --cfg ./experiments/supernet/supernet-T.yaml --epochs 410 --warmup-epochs 20 \
--resume '/OUTPUT_PATH/checkpoint-original-0.pth' --output /OUTPUT_PATH --batch-size 128 \
--save_checkpoint_path 'checkpoint-tiny-only-supernet-minimum-' --save_log_path './log/gradient_analysis.log'

# checkpoint-tiny-only-supernet-maximum240-20.pth
