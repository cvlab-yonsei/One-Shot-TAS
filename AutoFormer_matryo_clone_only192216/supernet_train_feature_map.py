...
from supernet_engine_feature_map import evaluate
...
sys.stdout = open('observe_supernet.log', 'w')
...
class ModelVisualizer(object):
    ...
    def load_checkpoint(self):
        ...
        # Fix for missing 'module.' prefix if using DistributedDataParallel
    ...
    def visualize_attention(self):
        ...
        image = images[80].unsqueeze(0).to(self.device)
        
        # Forward pass through the model to get raw output and attention
        # with torch.no_grad():
        #     output, attention_maps = self.model(image)
        
        # eval_stats = evaluate(self.val_loader, self.model, self.device, amp=self.args.amp, choices=self.choices, mode='super')
        eval_stats = evaluate(self.val_loader, self.model, self.device, image, net=self.model_without_ddp, amp=self.args.amp, choices=self.choices, mode='super')

        # print("Model output:", output)
        
        # # Visualize the attention maps
        # self.plot_attention(image.cpu().squeeze(), attention_maps.cpu().squeeze())

    def plot_attention(self, image, attention_maps):
        ...
        for i, attention_map in enumerate(attention_maps):
            ...
            axs[i + 1].imshow(attention_map, cmap='jet', alpha=0.6)
            axs[i + 1].axis('off')
...
def get_args_parser():
    ...
    parser.add_argument('--population-num', type=int, default=200) # 50 -> 1000?
...
def main(args):
    ...
    t = time.time()
    visualizer = ModelVisualizer(args, device, model, model_without_ddp, choices, data_loader_val, data_loader_test, args.output_dir)

    visualizer.load_checkpoint()
