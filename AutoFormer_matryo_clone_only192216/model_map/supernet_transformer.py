...
from model_map.module.Linear_super import LinearSuper
from model_map.module.layernorm_super import LayerNormSuper
from model_map.module.multihead_super import AttentionSuper
from model_map.module.embedding_super import PatchembedSuper
from model_map.utils import trunc_normal_
from model_map.utils import DropPath
import numpy as np
import random
...
class Vision_TransformerSuper(nn.Module):
    ...
    def set_sample_config(self, config: dict):
        ...
        sampled_block_indices = random.sample(range(len(self.blocks)), self.sample_layer_num)
    
        for i, blocks in enumerate(self.blocks):
            # not exceed sample layer number
            if i in sampled_block_indices:
                idx = sampled_block_indices.index(i)
                sample_dropout = calc_dropout(self.super_dropout, self.sample_embed_dim[idx], self.super_embed_dim)
                sample_attn_dropout = calc_dropout(self.super_attn_dropout, self.sample_embed_dim[idx], self.super_embed_dim)
                blocks.set_sample_config(is_identity_layer=False,
                                        sample_embed_dim=self.sample_embed_dim[idx],
                                        sample_mlp_ratio=self.sample_mlp_ratio[idx],
                                        sample_num_heads=self.sample_num_heads[idx],
                                        sample_dropout=sample_dropout,
                                        sample_out_dim=self.sample_output_dim[idx],
