...
#     # 설정값
#     warmup_epochs = 2                     # 각 구간의 warmup 에폭 수 (고정 2로 설정)
#     warmup_start_lr = args.warmup_lr      # ex. 1e-6
#     base_lr = args.lr                     # ex. 5e-4
#     min_lr = args.min_lr                  # ex. 1e-5

#     # 각 half range
#     total_epochs = args.epochs            # ex. 100
#     half = total_epochs // 2             # ex. 50

#     # 현재 절반 구간 기준 offset 계산
#     if epoch < half:
#         # 첫 번째 구간: 0 ~ 49
#         epoch_in_half = epoch
#     else:
#         # 두 번째 구간: 50 ~ 99 → epoch_in_half: 0 ~ 49
#         epoch_in_half = epoch - half

#     if epoch_in_half < warmup_epochs:
#         # Warmup (선형 증가)
#         current_lr = warmup_start_lr + (base_lr - warmup_start_lr) * (epoch_in_half / warmup_epochs)
#     else:
#         # Cosine decay
