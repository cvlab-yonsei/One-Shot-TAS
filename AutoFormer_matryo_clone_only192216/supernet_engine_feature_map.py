...
@torch.no_grad()
def evaluate(data_loader, model, device, image=None, amp=True, choices=None, net=None, mode='super', retrain_config=None):
    ...
    # ① 이미지 파일 경로 지정
    image_path = '/data/val/n04596742/ILSVRC2012_val_00001328.JPEG'  # or .png, .jpeg 등

    # ② 이미지 열기 (PIL.Image 형태)
    pil_image = Image.open(image_path).convert('RGB')

    # ③ ToTensor: [0, 1] 범위로 변환 + CxHxW 형태로 바꿈
    transform = transforms.ToTensor()
    image = transform(pil_image)  # shape: [3, H, W]

    print('image shape : ', image.shape)
    
    image = image.to(device, non_blocking=True)
    image_test = (image - image.min()) / (image.max() - image.min())  # 값 범위를 [0, 1]로 조정
    save_tensor_as_png(image_test, 'input_image1.png')
    ...
    # net.set_sample_config(config=config)  # 이걸 반드시 해줘야 함!
