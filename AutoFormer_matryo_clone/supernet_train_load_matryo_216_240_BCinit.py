...
from weight_clone_autoformer_matryoBC import copy_case2_parameters_matryo
...
def get_args_parser():
    ...
    parser.add_argument('--lr', type=float, default=0.00005, metavar='LR', # 5e-4 -> 0.001
    ...
    parser.add_argument('--min-lr', type=float, default=1e-5, metavar='LR', # 1e-5 -> 0.00001
...
def main(args):
    ...
    # 4-1. resume2로부터 또 다른 pretrained SuperNet(Matryo 클래스) 모델 로드
    if args.resume2:
        model_super = Vision_TransformerSuper_Matryo(
            img_size=args.input_size,
            patch_size=args.patch_size,
            embed_dim=cfg.SUPERNET.EMBED_DIM,
            depth=cfg.SUPERNET.DEPTH,
        ...
        # model_matryo 설정 및 selective 복사
        case_num = 2
        config = {
            'embed_dim': [240] * cfg.SUPERNET.DEPTH,
            'mlp_ratio': [4.0] * cfg.SUPERNET.DEPTH,
            'num_heads': [4] * cfg.SUPERNET.DEPTH,
            'layer_num': cfg.SUPERNET.DEPTH
        }

        model_module = unwrap_model(model_matryo)
        model_module.set_sample_config(config=config, case_num=case_num)

        copy_case2_parameters_matryo(model_super, model_matryo)
