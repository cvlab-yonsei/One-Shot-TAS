...
def sample_config_from_topk(model: torch.nn.Module, choices: Dict, m: int, k: int, device: torch.device, 
    ...
    with torch.no_grad():
        ...
        for config in sampled_paths:
            ...
            for layer in model.modules():
                ...
                if isinstance(layer, torch.nn.Linear) and layer.samples and norm_val != 0:
    ...
    for group_id in range(5):
        ...
        group_losses.sort(key=lambda x: x[0], reverse=False)
    ...
    if len(top_k_paths) < k:
        remaining_items.sort(key=lambda x: x[0], reverse=False)
