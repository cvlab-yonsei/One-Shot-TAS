...
class EvolutionSearcher(object):
    ...
    def load_candidates_from_pkl(self, pkl_path, num):
        ...
        If the `.pkl` file does not have enough valid candidates, random candidates are generated to fill the gap.
        ...
        def stack_loaded_cand(candidates, *, batchsize=10):
            ...
            """
            idx = 0
            while idx < len(candidates):  # Stop after one full iteration
        ...
        # ✅ 1. .pkl 파일에서 후보를 먼저 채운다
        while len(self.candidates) < num:
            try:
                cand_tuple, cand_dict = next(cand_iter)  # 다음 후보 가져오기
                if not self.is_legal(cand_tuple):  # 적합한지 검사
                    continue
                self.candidates.append(cand_tuple)  # 후보 리스트에 추가
                print('loaded {}/{}'.format(len(self.candidates), num))
            except StopIteration:
                print("⚠️ Not enough valid candidates in .pkl file. Switching to random selection...")
                break  # .pkl 후보 다 돌았으면 탈출

        # ✅ 2. 부족한 후보들을 랜덤으로 추가
        if len(self.candidates) < num:
            cand_iter = self.stack_random_cand(self.get_random_cand)  # 새로운 랜덤 샘플링 iterator 생성
            while len(self.candidates) < num:
                cand = next(cand_iter)  # 랜덤 후보 가져오기
                if not self.is_legal(cand):
                    continue
                self.candidates.append(cand)
                print('random {}/{}'.format(len(self.candidates), num))

        print('Final loaded_num = {}'.format(len(self.candidates)))
...
def get_args_parser():
    ...
    parser.add_argument('--config-list-path', default='', type=str, help='Path to the config list pickle file')
