...
class EvolutionSearcher(object):
    ...
    def __init__(self, args, device, model, model_without_ddp, choices, val_loader, test_loader, output_dir):
        ...
        self.config_list = self.load_config_list(args.config_list_path)  # pkl 파일에서 config 리스트 로드

    def load_config_list(self, path):
    ...
    # random_cand, mutation, crossover 관련 코드들은 주석처리
    ...
    def search(self):
        print('Loading and evaluating predefined config list...')
        for cand in self.config_list:
            # cand가 dict 형태로 되어 있으므로, 필요한 값을 추출하여 tuple로 변환
            depth = cand['layer_num']
            mlp_ratio = cand['mlp_ratio']
            num_heads = cand['num_heads']
            embed_dim = cand['embed_dim']
            
            # tuple 형태로 변환 (depth, mlp_ratio, num_heads, embed_dim)
            cand_tuple = (depth, *mlp_ratio, *num_heads, embed_dim[0])

            # 변환된 tuple을 사용하여 is_legal 함수 호출
            self.is_legal(cand_tuple)
